...
var element = function element(tag) {
  ...
  if (classes.length) {
    var _$node$classList;

    (_$node$classList = $node.classList).add.apply(_$node$classList, _toConsumableArray(classes));
  }
...
function upload(selector) {
  ...
  var trigger$input = function trigger$input() {
    return $input.click();
  };
  ...
  var clearaPreview = function clearaPreview(el) {
    el.style.bottom = '0px';
  ...
  onUpload: function onUpload(files, blocks) {
    files.forEach(function (file, index) {
      ...
      task.on('state_changed', function (snapshot) {
        ...
        return task.snapshot.ref.getDownloadURL().then(function (url) {
